...
class multitree:
	...
	elif colon_count == 3 and len(re.findall(':[^\)]*:[^\)]*:',newick)) > 0:
    ...
    def rootify(self,center_branch):
        ...
        center_node = node_mas.node(center_name,self)
        self.root = center_node
        
        # give it children branches
        child1 = branch_mas.branch(center_branch.length/2)
        child1.addNode(center_node)
        child1.addNode(center_branch.ends[0])
        child2 = branch_mas.branch(center_branch.length/2)
